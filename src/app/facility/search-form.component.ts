...
class SearchFormComponent 
{
    ...
    onSubmit(search: Search)
    {
        ...
        if (search.criteria === 'hucCode') {
                ...
                .subscribe(
                    (data: any) => {
                        if (data.facilityData === '[]') {
                            ...
                            console.log(JSON.parse(data.eweData));
                            if (JSON.parse(data.eweData).generation !== null) {
                                generations += Number(JSON.parse(data.eweData).generation);
                            }
                            if (JSON.parse(data.eweData).participating_facilities !== null) {
                                participating += Number(JSON.parse(data.eweData).participating_facilities);
                            }
                            if (JSON.parse(data.eweData).emission !== null) {
                                emissions += Number(JSON.parse(data.eweData).emission);
                            }
                            if (JSON.parse(data.eweData).waterUsage !== null) {
                                water_usage += Number(JSON.parse(data.eweData).waterUsage);
                            }
                            console.log(participating + emissions + generations + water_usage);
            ...
            this.mapsService.get_facility_json(search.criteria,
                                               search.value,
                                               search.match,
                                               search.start_year,
                                               search.end_year)
